% Script to apply Mmax tests to the Musreau Lake data.
% Used to make Figures 7, S5, & S6.
clear;
...
% Get the Musreau Lake catalogue.
load('data/MUSREAU_cat.mat','Lat','Lon','Dep','T','M','ID'); 
ID=ID'; Lat=Lat'; Lon=Lon'; Dep=Dep';
T=T'; M=M';
...
S(1).Mmax=(max(Mlrg)+0.1)*ones(size(Mlrg));
S(2).Mmax=(5.3)*ones(size(Mlrg));
...
function [id,x,y,z,t,m,I,m1]=filtCat(id,x,y,z,t,m)
  ...
  z_L=[0 10];
  y_L=[  54.482   54.502   54.502   54.482]; % Big fault.
  x_L=[-118.508 -118.508 -118.501 -118.501]; % Big fault.
  T_L=[datetime(1980,01,01) datetime(2024,01,01)];
